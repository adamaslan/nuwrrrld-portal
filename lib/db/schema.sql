...
-- stale_count }`.
ALTER TABLE pipeline_run_log ADD COLUMN IF NOT EXISTS host text;   -- 'gha' | 'modal' | 'gcp' | 'local'
ALTER TABLE pipeline_run_log ADD COLUMN IF NOT EXISTS status text; -- 'ok' | 'degraded' | 'partial' | 'fail'
...
CREATE OR REPLACE FUNCTION paper_orders_watchlist_guard() RETURNS trigger AS $$
BEGIN
  IF NEW.side = 'buy' AND NOT EXISTS (
    SELECT 1 FROM paper_watchlists
    WHERE account = NEW.account AND ticker = NEW.ticker AND active
  ) THEN
    RAISE EXCEPTION 'paper_orders: % is not on an active watchlist for account %',
      NEW.ticker, NEW.account;
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS paper_orders_watchlist_guard_trg ON paper_orders;
CREATE TRIGGER paper_orders_watchlist_guard_trg
  BEFORE INSERT ON paper_orders
  FOR EACH ROW EXECUTE FUNCTION paper_orders_watchlist_guard();
