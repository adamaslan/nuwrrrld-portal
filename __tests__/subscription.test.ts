...
describe("parseSubscriptionMetadata", () => {
  ...
  describe("admin override", () => {
    ...
    it("resolves pro for an allowlisted admin regardless of subscription_status", () => {
      const result = parseSubscriptionMetadata({ subscription_status: "canceled" }, admin());
      expect(result.status).toBe("canceled");
      expect(result.tier).toBe("pro");
    });

    it("does not override tier for a non-admin user", () => {
      const result = parseSubscriptionMetadata({ subscription_status: "canceled" }, admin("someone-else@example.com"));
      expect(result.tier).toBe("free");
    });

    it("does not override tier for an unverified admin address", () => {
      const result = parseSubscriptionMetadata({ subscription_status: "canceled" }, admin("admin@example.com", false));
      expect(result.tier).toBe("free");
    });
  });
});
