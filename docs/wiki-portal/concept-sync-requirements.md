...
| ~~`lib/shared/signal-policy.ts`~~ | **Re-synced — mobile PR #43, gcp-expo1 (2026-09-04).** Re-drifted by portal PR #101 (`isCryptoShaped()`, Phase 2.4 of the signal-engine plan); ported verbatim to mobile the same day. Byte-identical again, confirmed by diff. Mobile has no caller (no registration route) — the port exists solely to hold the shared-drift gate, same shape as prior signal-policy.ts re-syncs. |
...
| `lib/shared/fib-levels.ts` | New (portal PR #185), portal-only. Types the holdfold Fibonacci response and derives the ladder rows. Portable as-is, but mobile's `clients/holdfold.ts` declares `FibLevel { level, price }` (`level` never arrives): first fix that type to `{ name, price, distance_pct, strength, type }`, then adopt the module if mobile grows a Fibonacci surface. Do it via `/syncpr` so both wikis update together. |
...
| **Portfolio health provenance** ([[entity-portfolio-intelligence]] failure 5) | Read `X-Portfolio-Health-Source` in `lib/usePortfolio.ts` and render a "computed by the portal signal engine" line in `PortfolioScreen.tsx`, matching web's `.port-health-source`. Cheap (a header read + a label) and **higher priority than its size suggests**: mobile currently presents a portal-computed score as though it came from the documented backend. New 2026-09-11 — and the first item on this list where the two surfaces diverge on *what the user is told* rather than on code, so `shared-drift-check` cannot see it. Adding a response field one client reads is a cross-surface contract change even when no `lib/shared/` file moves. |
| **Effective-tier overrides** ([[entity-billing]]) | Mobile derives tier from its own `lib/subscription.ts` + Clerk metadata, so it never sees the portal's `resolveTier()` overrides — an allowlisted admin or beta tester is **Pro on web, Free on mobile**. Cheapest fix is *not* porting `lib/subscription-admin.ts`/`lib/beta-testers.ts` (which would need the same env/allowlist on a client bundle — a non-starter for the allowlist and wrong for a mobile binary): have `useSubscription` read `GET /api/stripe/subscription`, which already returns the override-aware tier from `parseSubscriptionMetadataWithAdmin()`. New 2026-09-13. Second instance of the class the row above opened — divergence in *outcome* with no shared file touched, invisible to `shared-drift-check`. |
