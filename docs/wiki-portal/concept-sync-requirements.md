...
| `lib/shared/signal-policy.ts` | **Re-drifted — portal PR #101 (2026-09-03).** Was byte-identical since mobile PR #32; PR #101 adds `isCryptoShaped()` (rejects `*-USD`-style pairs at the portal's `PUT` registration route, Phase 2.4 of the signal-engine plan). No mobile caller — mobile has no registration route — but the module is single-sourced, so mobile must port the addition to restore byte-identity, or accept the drift. Small, pure, zero-dependency; a one-line port. |
