...
## Headline: ~62% synced (2026-08-29, after the auth/DSAR/analytics PR — data-subject rights land web-only, +1 portal-only `lib/shared/` module)

> ℹ️ **Portal PR (2026-08-29) assessed — auth hardening + DSAR rights + analytics/attribution scaffolding, headline ~63% → ~62%.** Builds directly on PR #77's consent branch (merged in locally, since Phases 3–7 import `lib/shared/consent.ts` and waiting for #77's red CI would have blocked all of them). Three parity-relevant facts. **First**, one new `lib/shared/` module — `attribution.ts`, the first-party acquisition model (`nu_attrib` cookie, UTM/gclid/fbclid/referrer, 90-day first-touch). It is written to be mirrored and has no mobile counterpart, so 16 shared modules now to mobile's 5: single-source ~40% → ~38%. **Second**, the auth-hardening work is structurally portal-only and *should* stay that way — `middleware.ts` edge-matcher coverage and the `lib/http-auth.ts` timing-safe bearer compare guard server routes mobile does not have. Worth recording that `lib/http-auth.ts` is deliberately pure-JS rather than `node:crypto`: the Edge Middleware bundle cannot load `node:crypto`, and `next build` surfaced that as an Ecmascript error *while still exiting 0* — a build-passes-but-is-broken shape worth remembering. **Third and most consequential for parity**: the data-subject-rights surface (`/api/privacy/{export,profile,delete,rectify}`, the `privacy_requests` statutory-clock ledger) and the analytics consent gate (`lib/analytics.ts`, which no-ops unless `nu_consent.analytics` is true) are now web-only. Combined with the contradiction the previous entry logged — mobile tracks with no consent gate — the shared-identity product now has a **second** compliance asymmetry: a user can exercise GDPR access/erasure/rectification on web but has no mechanism on mobile, against the same account. That is a feature-domain gap, not just a code-sharing one, so the feature denominator moves too. `docs/privacy-register.md` (cookie table, processor register, retention targets, Art. 6 bases) is the shared input both surfaces' policies should generate from — keeping them identical in substance is a compliance requirement, not a copy nit. See [[concept-sync-requirements]]: mobile adopting `lib/shared/consent.ts` is now priority #1, with `attribution.ts` and a mobile DSAR path behind it.
