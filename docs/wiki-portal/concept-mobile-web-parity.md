...
> ℹ️ **Portal PR #88 (2026-08-31) assessed — followed-tickers eval-harness backend, headline unchanged at ~62%.** Lands follow-up items 1–8 of `docs/tickers-followed.md` behind the PR #85 workflows: the 3-table schema (`followed_ticker_picks`/`_observations`/`_scores`), the three `/api/pipeline/followed-tickers*` routes (monthly select, daily observer + horizon resolution, weekly gold-gated judge), the pure outcome math (`lib/eval-scoring.ts` — per-horizon dead-band, n<30 suppression, four baselines), the judge rubric + gold-set gate (`lib/eval-judge.ts`), the store access + doc-section rewriter (`lib/followed-tickers-db.ts` / `lib/followed-tickers-render.ts`), and pure cohort selection (`lib/shared/followed-tickers-policy.ts`). Same portal-only shape as #85: none of it is mobile-reachable — mobile has no ticker universe, no council-grounding path, no backtest engine, and no server-side cron. `lib/shared/followed-tickers-policy.ts` is a *new* `lib/shared/` module but a portal-only one with no mobile counterpart or candidate, exactly like `universe-policy.ts` (PR #71) and `analyze-policy.ts` (PR #56) before it — the single-source denominator is unchanged (15 portal / 5 mobile), and the feature denominator does not move either. Portable content is methodology only: outcome-vs-reasoning as separate axes, the n<30 publish floor, baseline-relative scoring.

> ℹ️ **Portal PR #89 (2026-08-31) assessed — portal-only billing repair, headline unchanged at ~62%.** A live-account audit of [[entity-billing]] found checkout broken on *both* plans, not just annual. `STRIPE_PRICE_ANNUAL` still held the literal `price_annual_placeholder` — the same defect **portal PR #79 recorded as repaired ten days earlier**, which is the finding worth carrying forward: #79 fixed the code path and the docs but the value never reached `.env.local`, so the repair was invisible to the running app and no test caught it. `STRIPE_PRICE_MONTHLY` was worse and undocumented — it pointed at an `active: false` archived $10.00 price, so the monthly plan had been unpurchasable too. Both now point at live active IDs sharing the `NuWrrrld_Monthly` product ($9.99/mo, $79.99/yr), which matches the "Save 33%" copy already hardcoded across `app/pricing` and `app/dashboard/{upgrade,billing}` — the UI was correct and only the environment was stale. Separately, the account's **only** live webhook endpoint targets the `gcp3-backend` Cloud Run service, not the portal: nothing is registered for `/api/webhooks/stripe`, so the long-missing `STRIPE_WEBHOOK_SECRET` was never the real blocker — no events were reaching the portal at all, and the existing endpoint omits `customer.subscription.created`, which the portal's handler switches on. `scripts/create-stripe-webhook.sh` provisions the endpoint and captures the once-shown `whsec_…` directly into `.env.local`. **Parity impact: none.** `lib/subscription.ts` is untouched and stays byte-identical; the changed surface is portal-only env wiring and provisioning tooling. Mobile reads entitlement through the same Clerk identity and has no Stripe price IDs or webhook endpoint of its own, so neither denominator moves. The transferable lesson is a process one, not a code one: **a price ID marked "fixed" in a doc is not fixed until the deployed environment holds it** — worth checking against mobile's own store-billing config, which has never been audited this way.

> ℹ️ **Portal PR #97 (2026-09-02) assessed — portal-only AI Council infrastructure, headline unchanged at ~62%.** Same shape as PR #75: fixes to `lib/openrouter.ts` ([[entity-openrouter-client]] failures #8 and #9 — a rotted `SEAT_MODELS.QUANT` id, and `runSeat` accepting an HTTP 200 with an empty completion as a successful answer) plus a landing-page change to `/api/council/sample` (the unauthenticated `#council` demo now simulates a $10,000 MOO position instead of a bare SPY prompt). `lib/openrouter.ts` is portal-only — mobile's council talks to gcp3, not OpenRouter directly ([[entity-ai-council]] "Where used") — so there is nothing to port, no `lib/shared/` module is touched, and the landing sample is a portal-only marketing surface with no mobile screen rendering it. Neither denominator moves. The transferable content is the same *class* of finding PR #75 recorded: a fallback chain's habit of degrading silently (an empty completion looks identical to a real answer; a 404'd primary looks identical to the fallback chain working as designed) means the fix that resolved failure #5 a month ago did not prevent its recurrence as failure #8 — the weekly `refresh-free-models.mjs` audit exists but nothing runs it on a schedule, an open item still unfixed.

> ⚠️ **Cross-wiki drift (pre-existing, noted 2026-08-31 during PR #88 ingest):** this wiki's headline sits at **~62%** (portal PRs #77/#78/#79 dropped it from ~66% via the web-only DSAR/consent asymmetry); `gcp3-mobile/docs/wiki-mobile/concept-mobile-web-parity.md` still reads **~66%** (last recomputed 2026-08-08). The two need re-mirroring — a `wiki-mobile` change, out of scope for this portal PR, tracked here so it is not lost.

> ⚠️ **Portal PR #101 (2026-09-03) assessed — signal-engine three-phase plan, code-only Phases 1–3. Headline unchanged at ~62%; one *synced* shared module drifts.** Lands the code slice of `docs/signal-engine-three-phase-plan.md` on [[entity-ticker-universe-pipeline]]: loud failure alerting + an out-of-writer freshness check (`signal-freshness-check.yml`, `GET ?meta=freshness`), Alpaca `next_page_token` pagination in `hydrate-local.mjs`, `lib/shared/hydration-constants.json` as the single source for the hydration constants, `BRK-B`→`BRK.B` symbology normalization at ingest, an `isCryptoShaped()` reject at the `PUT` registration route, and a real `dataQuality` (`completeness × barQuality(frameStats)`) in `lib/shared/card-policy.ts`. **`card-policy.ts` and `hydration-constants.*` are portal-only** — mobile has no ticker universe, hydration lane, or card ranking — so they are not parity candidates, same shape as `universe-policy.ts` (#71). **The drift is in `lib/shared/signal-policy.ts`**, which mobile adopted *verbatim* (mobile PR #32) and which the drift gate keeps byte-identical: this PR adds `isCryptoShaped()` to it. The function has no mobile caller (mobile has no registration route), but the module is single-sourced, so mobile must either port the addition to restore byte-identity or the two copies drift. Feature-domain parity is untouched — no user-visible mobile surface changed — so the blended headline holds, exactly as PR #71/#88 recorded for portal-only shared additions; the difference here is that the file itself was already in the synced set. Portal is now at **~19** `lib/shared/` modules to mobile's **5**. Tracked in [[concept-sync-requirements]].

> ✅ **Mobile PR #43, gcp-expo1 (2026-09-04) — closes the drift PR #101 opened, same day.** Ports `isCryptoShaped()` verbatim into mobile's `lib/shared/signal-policy.ts`, restoring byte-identity (confirmed by diff, not just by intent). No mobile caller exists — mobile has no ticker-registration route — so this is a pure drift-gate-holding port, the same shape mobile PR #32 established for this file originally. Verified with a clean `tsc --noEmit` rather than a mirrored test, since `gcp3-mobile`/`gcp-expo1` has no test runner configured at all (no vitest/jest, no `test` script) — the portal's `__tests__/signal-policy-crypto.test.ts` was deliberately not copied over, as it would have been dead code. Neither denominator moves; this restores existing single-source parity rather than extending it.
> ℹ️ **Portal PR #108 (2026-09-04) assessed — landing-page-only fix, headline unchanged at ~62%.** The signed-out landing mockup fetched a live `/market-overview` MCP response and re-keyed it by symbol for the S&P/QQQ/DIA/IWM mini-grid — the same unscoped, ~16s-class endpoint PR #46 found costing `/api/brief` its timeout. Pre-signup, that fetch regularly returned nothing useful, so the mockup showed a loading/empty state instead of a snapshot. Replaced with static, labeled sample data ("Sample data — illustrative only"); removes `indexBySymbol()` and the market-overview fetch from `fetchLandingData()` entirely. **No `lib/shared/` module touched, no mobile counterpart** — this is the signed-out marketing page, not a screen mobile has; mobile's own `getMarketOverview()` unscoped-fetch cost (noted in [[concept-sync-requirements]] §2) is unaffected and still open. Neither denominator moves.
> ℹ️ **Portal PR #113 (2026-09-06) assessed — landing-page CSS only, headline unchanged at ~62%.** Three contrast defects on the signed-out landing: the `--dim` token measured 4.12:1 on `--bg` (under the 4.5:1 AA floor) across all ten of its consumers; the [[entity-ai-council]] scroll-debate's seat labels used that token *multiplied by* the reveal opacity, compositing to 2.00:1; and the component's `prefers-reduced-motion` reset never applied, because a media query adds no specificity and its two-class selector lost to the three-class `.is-revealed` rule — pinning every line dim for reduced-motion users permanently. See [[concept-composited-contrast]]. **No `lib/shared/` module touched and no mobile counterpart exists**: this is the signed-out marketing page, and React Native has no CSS cascade, no `@media` queries, and no `--dim` token to share. Same shape as portal PR #108 and the PR #42/#43 landing revamps before it — neither denominator moves.
> ℹ️ **Portal PR #118 (2026-09-10) assessed — admin/test tooling only, headline unchanged at ~62%.** Repairs [[entity-nulogdash]]'s inventory (13 routes had no `FEATURE_META` entry, so a cookieless sweep scored them `fail` instead of `blocked` — six of them because Clerk answers an unauthenticated request to a proxy-matched API route with `404` rather than `401`), and adds `e2e/frontend/nulogdash-admin.spec.ts` to [[entity-playwright-e2e]]. **No `lib/shared/` module touched and none added; no `app/` behavior changed** — the only source edits are a build-time inventory script and a new read-only test spec. Mobile has no admin console, no feature-sweep inventory, and (still) no test runner configured at all, so there is nothing to port and no gap created. Same shape as PR #64, which added the Playwright suite itself, and PRs #59/#74's scheduler tooling: portal-only, neither denominator moves. The transferable piece is a *principle* rather than code — a pure gate function extracted for testability is testable without its caller, so unit coverage proves the predicate correct while saying nothing about whether it is reached ([[concept-test-strategy]]) — which would apply to any mobile auth gate, but mobile has no equivalent surface today.
