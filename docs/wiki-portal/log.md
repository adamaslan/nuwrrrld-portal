...
`.github/workflows/hydrate-universe.yml` runs it weekdays at 21:30 UTC — 17:30 ET, ~75 minutes after the close. The settle margin is deliberate rather than arbitrary: Alpaca's daily bar is not final at the bell, and a run that beats the vendor to its own data would card every symbol against a partial last bar. Weekdays only, since a weekend run re-cards Friday and burns vendor calls writing rows `shouldReplaceCard` correctly refuses to replace.
