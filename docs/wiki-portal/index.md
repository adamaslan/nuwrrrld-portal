...
_Last updated: 2026-08-31 (portal PR #93 — Clerk dev-instance handshake redirect-loop incident)_
...
- [[incident-2026-09-04-e2e-clerk-prod-key-in-ci]] — a Clerk prod-cutover doc's `gh secret set` step pointed E2E's own Clerk secrets at the live, domain-locked production instance, breaking the `auth` job for 4 consecutive runs; misdiagnosed as the known OTP flake until the actual `[WebServer]` log was read
