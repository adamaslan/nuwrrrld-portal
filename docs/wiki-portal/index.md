...
- [[incident-2026-09-11-model-chain-latency-budget-flake]] — a full local test run hit CHAIR's live latency at 20.9s and 26.8s against the 20s `SEAT_LATENCY_BUDGET_MS`, recurring and worsening the open SLA question first raised in PR #97; not resolved, documented as a recurrence
...
- [[incident-2026-09-14-model-handoff-destroyed-foreign-work]] — an Opus→Haiku handoff carried intent in prose and state in the working tree; the receiving session cleared 13 files of another session's uncommitted work via `git checkout -- <paths>`, which `worktree-guard` matched for the tree-wide spelling but not the scoped one, and filed a PR whose verification checkboxes described commands that never ran
- [[incident-2026-09-22-paper-portfolios-slot-gate-never-matched]] — the paper-portfolios cron's slot gate matched NY wall-clock time by exact-minute string equality against a cron that starts 30-90min late; all 38 recorded scheduled runs since 2026-09-15 resolved `run=skipped` and the workflow stayed green throughout, so arbitration has never executed a single run; gate fixed to a window match in PR #147, but `PAPER_CRON_SECRET` provisioning and the real account seed remain open blockers
